...
program
  .name('grid-cli')
  .description('CLI tool for grid selection component')
  .version('1.0.0')

program
  .command('create')
  .description('Generate grid component with specified dimensions')
  ...
  .action(async (options) => {
    try {
      ...
      if (isNaN(cols) || isNaN(rws) || cols <= 0 || rws <= 0) {
        throw new Error('Columns and rows must be positive numbers')
      }

      console.log(chalk.blue(`Generating grid with ${cols}x${rws} dimensions...`))
      
      // Generate component
      const component = generateGridComponent(cols, rws)
      
      // Save to file
      const outputPath = path.join(process.cwd(), 'grid-component.js')
      fs.writeFileSync(outputPath, component)
      
      console.log(chalk.green('Grid component generated successfully!'))
      console.log(chalk.blue('File created:', outputPath))
    } catch (error) {
...
function generateGridComponent(columns, rows) {
  return `// Wait for DOMQL to be available
const waitForDOMQL = () => {
...
export const GridSelection = ({ columns = ${columns}, rows = ${rows} }) => {
  const state = {
    selectedCells: {},
    clickedCell: null
  };
